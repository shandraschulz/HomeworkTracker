...
function uploadBackupFile(file) {
  ...
  reader.onload = () => {
    try {
      ...
      if (!confirm("This will replace your current tracker data in this browser. Continue?")) return;

      const restored = normalizeState(s);
      // Replace in-memory state entirely
      state = restored;
      saveState();
      render();
    } catch (err) {
...
function taskRow(task, classId) {
  ...
  cb.onchange = (e) => { e.stopPropagation(); task.done = cb.checked; render(); };
  ...
  del.onclick = (e) => { e.stopPropagation(); removeTask(classId, task.id); };
...
function addClass(name) {
  ...
  const newCls = { id: uid(), name: trimmed };
...
function addTask(title, dueDateISO) {
  const cls = activeClass();
  if (!cls) return;
...
function removeTask(classId, taskId) {
...
function clearCompleted() {
  ...
  const tasks = state.tasksByClass[cls.id] || [];
  const toHide = tasks.filter(t => t.done && !t.hidden);
  if (toHide.length === 0) return;

  // mark them hidden but keep in storage
  toHide.forEach(t => { t.hidden = true; });

  // record last clear for undo (in-memory only)
  lastClear = { classId: cls.id, taskIds: toHide.map(t => t.id) };
